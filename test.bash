...
output=(
	...
	"`./ft_ssl sha256 -q test.txt`"
	"`echo '' | ./ft_ssl sha256 -q`"
	"`echo '' > file && ./ft_ssl sha256 file -q`"
)
...
expected=(
	...
	"`openssl sha256 test.txt | awk '{print $2}'`"
	"`echo '' | openssl sha256 | awk '{print $2}'`"
	"`echo '' > file && openssl sha256 file | awk '{print $2}'`"
)
